...
getExistingProject() {
  if [ "$DEVOPS_TARGET" == "ADO" ]; then
    DEVOPS_PROJECT_ID=$(az devops project show -p $DEVOPS_PROJECT --org $DEVOPS_ORG --query 'id' -o tsv)
  fi
}
...
getRepoURL() {
  if [ "$DEVOPS_TARGET" == "ADO" ]; then
    REPO=$(az repos show -r $DEVOPS_PROJECT --project $DEVOPS_PROJECT --org $DEVOPS_ORG --query webUrl -o tsv)
  fi
  echo "REPO=$REPO"
}
...
getADFRepoURL() {
  if [ "$DEVOPS_TARGET" == "ADO" ]; then
    ADFREPO=$(az repos show -r adf-config --project $DEVOPS_PROJECT --org $DEVOPS_ORG --query webUrl -o tsv)
  fi
...
if [ "$DEVOPS_TARGET" == "ADO" ]; then
  ...
  if [ ${#GIT_BRANCH[@]} -gt 1 ]; then
    REPO_ID=$(az repos show -r $DEVOPS_PROJECT --org $DEVOPS_ORG --project $DEVOPS_PROJECT --query id -o tsv)
  ...
  SE_ID=$(az devops service-endpoint list -p $DEVOPS_PROJECT --org $DEVOPS_ORG --query "[?name=='azure-sp'].id" -o tsv)
  if [ -z $SE_ID ]; then
